    ...
    <ValidationObserver ref="observer">
      ...
      <form @submit.prevent="submit">
        <ValidationProvider name="email" rules="required|email" v-slot="{errors}">
          ...
          <span v-if="errors.length > 0" style="color: coral">{{ errorMessage(errors[0]) }}</span>
        </ValidationProvider>
        <ValidationProvider name="password" rules="required|min:6|integer" v-slot="{errors}">
          <input v-model="password" placeholder="type something" />
          <span v-if="errors.length > 0" style="color: coral">{{ errorMessage(errors[0]) }}</span>
        </ValidationProvider>
        <button @submit.prevent="submit" style="margin: 20px auto 0 auto; display: block">
          submit
        </button>
      </form>
    </ValidationObserver>
